...
class SpectralTrainer:
    ...
    def train(
        ...
        # W = self._get_affinity_matrix(self.X)
        # L = torch.diag(W.sum(dim=1)) - W
        # _, V = torch.linalg.eigh(L.detach().cpu())
        # V = V.detach().cpu().numpy()
        # torch.save(V, 'eigenvectors_of_flickr30_trainset.pt')
        # V = torch.load('eigenvectors_of_flickr30_trainset.pt')
        ...
        for epoch in t:
            ...
            ####### Compute Grassmann #######
            # Y = self.spectral_net(self.X, should_update_orth_weights=False)
            # Y = 1 / np.sqrt(Y.shape[0]) * Y
            # n_eigenvectors = Y.shape[1]
            # V = V[:, :n_eigenvectors]
            # grassmann = get_grassman_distance(V, Y.detach().cpu().numpy())
            # print(grassmann)
            #################################
